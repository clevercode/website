...
#
class Slide
  constructor: (@element, @index, @options, @numberOfSlides) ->

    # ZWH
    # @element.css { position: 'absolute', top: 0, left: @index * @element.width(), width: @element.width() }
    @element.css
      top: 0
      position: 'absolute'
...
class Slider
  ...
  initSlides: ->
    ...
    @container.children().each (index, element) => 
       @slides.push(new Slide($(element), index, @options, @numberOfSlides))

    # ZWH
    # @container.css('width', @size.width * @slides.length)
    @container.css('width', (100 * @slides.length) + .1 + '%')
