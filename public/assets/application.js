      ...
      if (o.fluid) {
        $(window).resize($.proxy(function() {
          ...
          this.rs = setTimeout($.proxy(function() {
            ...
            this.max[0] = width;
            this.ul.css(styl);
            styl['width'] = Math.min(Math.round((width / this.el.parent().width()) * 100), 100) + '%';
            this.el.css(styl);
          }, this), 50);
        }, this)).resize();
      }
...
(function() {
  ...
  window.cleverCode = {
    init: function() {
      this.page = $('header');
      return this.page.find('#mantra').fitText(1.2);
    }
  };
  ...
  $(function() {
    ...
    return cleverCode.contact.init();
  });
...
(function() {
  ...
  cleverCode.contact = {
    init: function() {
      this.page = $('#content.contact');
      if (!this.page.length) {
        return;
      }
      this.attachEventListeners();
      return this.page.find('#cleverContact_name').focus();
    },
    attachEventListeners: function() {
      this.page.on('focus', 'input, textarea', $.proxy(this, '_onInputFocus'));
      this.page.on('blur', 'input, textarea', $.proxy(this, '_onInputBlur'));
      return this.page.on('submit', 'form', $.proxy(this, '_onFormSubmit'));
    },
    _onInputFocus: function(event) {
      return $(event.currentTarget).parent().addClass('focus');
    },
    _onInputBlur: function(event) {
      ...
      if ($input.val().length > 0) {
        return $input.parent().addClass('completed');
      }
    },
    _onFormSubmit: function(event) {
      var $input, $inputs, input, _i, _len;
      this.page.find('li.action input').prop('disabled', 'disabled');
      $inputs = this.page.find('li.input input, li.input textarea');
      for (_i = 0, _len = $inputs.length; _i < _len; _i++) {
        input = $inputs[_i];
        $input = $(input);
        if ($input.val().length === 0) {
          $input.focus().parent().addClass('error');
          this.page.find('li.action input').removeAttr('disabled');
          return false;
        }
      }
      this._sendEmail($(event.currentTarget).serializeArray());
      return false;
    },
    _sendEmail: function(data) {
      var call, email, message, name, params,
        _this = this;
      name = _.find(data, function(item) {
        return -1 < item.name.search('name');
      });
      email = _.find(data, function(item) {
        return -1 < item.name.search('email');
      });
      message = _.find(data, function(item) {
        return -1 < item.name.search('message');
      });
      params = {
        name: name.value,
        email: email.value,
      ...
      call.done(function(data) {
        return _this.page.find('form').css({
          opacity: 0
        });
      });
      return call.fail(function(data) {
        return _this.page.find('li.action input').removeAttr('disabled');
      });
    }
  };
...
(function() {
  ...
  cleverCode.home = {
    PROJECTS: ['ruby-heroes', 'roomies', 'firehouse', 'feedpage'],
    init: function() {
      this.page = $('#content.home');
      if (!this.page.length) {
        return;
      }
      this.page.find('.team h1').slabText({
    ...
    attachEventListeners: function() {
      return this.page.on('click', 'ul.gallery > li', $.proxy(this, '_onGalleryClick'));
    },
    _onGalleryClick: function(event) {
      var project;
      project = this.PROJECTS[this._slider.i];
      return window.location = '/work#' + project;
    },
    _initSlider: function() {
      var slider;
      slider = this.page.find('.image-container').unslider({
        arrows: true,
        delay: 3000,
        dots: true,
        fluid: true,
      ...
      return this._slider = slider.data('unslider');
    }
  };
...
(function() {
  ...
  cleverCode.work = {
    PROJECTS: ['ruby-heroes', 'roomies', 'firehouse', 'feedpage'],
    init: function() {
      this.page = $('#content.work');
      if (!this.page.length) {
        return;
      }
      this.attachEventListeners();
      this._initSlider();
      return this._navigateToSlide();
    },
    attachEventListeners: function() {},
    _initSlider: function() {
      var slider;
      slider = this.page.find('.image-container').unslider({
        ...
        keys: true,
        pause: true,
        before: $.proxy(this, '_beforeSliderTransition'),
        after: $.proxy(this, '_afterSliderTransition')
      });
      this._slider = slider.data('unslider');
      return this._slider.stop();
    },
    _beforeSliderTransition: function() {
      this._project = this.PROJECTS[this._slider.i];
      return this.page.find('.case-study').attr('data-project', this._project);
    },
    _afterSliderTransition: function() {
      return window.location.hash = this._project;
    },
    _navigateToSlide: function() {
      var _this = this;
      return setTimeout(function() {
        var project;
        project = window.location.hash.replace('#', '');
        if (_this.PROJECTS.indexOf(project) !== -1) {
          return _this._slider.to(_this.PROJECTS.indexOf(project));
        }
      }, 1100);
    }
  };
